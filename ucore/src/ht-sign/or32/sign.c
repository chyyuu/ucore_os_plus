...
{
	if (argc < 3) {
		printf ("usage: %s <dev/file name> <bootblock> [<part1> <part2> ...]\n", argv[0]);
		return 1;
	}
	...
	for (i = 3; i < argc; i++) {
		if (i >= 6) {
			printf ("warning: too much partitions. only the first four are handled.");
			break;
		}
	...
	for (i = 0; i < 4; i++) {
		if (parts[i] != 0) {
			...
			nr_unit = (next_block*BLOCK_SIZE + file_status.st_size - 1) / (PARTITION_UNIT * BLOCK_SIZE) + 1;
			...
			/* Now copy the files. */
			int j;
			actual_blocks = (file_status.st_size - 1) / BLOCK_SIZE + 1;
