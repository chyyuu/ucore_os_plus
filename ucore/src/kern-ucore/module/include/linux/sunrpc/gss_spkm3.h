...
u32 spkm3_make_token(struct spkm3_ctx *ctx, struct xdr_buf *text,
		     struct xdr_netobj *token, int toktype);

u32 spkm3_read_token(struct spkm3_ctx *ctx, struct xdr_netobj *read_token,
		     struct xdr_buf *message_buffer, int toktype);
...
void spkm3_make_mic_token(unsigned char **tokp, int toklen,
			  ...
			  struct xdr_netobj *md5cksum, int md5elen,
			  int md5zbit);
