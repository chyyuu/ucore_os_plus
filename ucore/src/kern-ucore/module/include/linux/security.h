...
struct security_operations {
	...
	int (*ptrace_may_access) (struct task_struct * child,
				  unsigned int mode);
...
void security_sb_post_remount(struct vfsmount *mnt, unsigned long flags,
			      void *data);
    ...
    int security_real_capable_noaudit(struct task_struct *tsk, int cap)
{
	...
	rcu_read_lock();
	ret = cap_capable(tsk, __task_cred(tsk), cap, SECURITY_CAP_NOAUDIT);
	rcu_read_unlock();
	return ret;
}
...
static inline int security_sb_check_sb(struct vfsmount *mnt, struct path *path)
{
	return 0;
}
