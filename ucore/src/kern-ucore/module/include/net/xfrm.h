...
struct xfrm_policy_afinfo {
	...
	int (*get_saddr) (struct net * net, xfrm_address_t * saddr,
			  xfrm_address_t * daddr);
	struct dst_entry *(*find_bundle) (struct flowi * fl,
					  struct xfrm_policy * policy);
	void (*decode_session) (struct sk_buff * skb, struct flowi * fl,
				int reverse);
	int (*get_tos) (struct flowi * fl);
	int (*init_path) (struct xfrm_dst * path,
			  struct dst_entry * dst, int nfheader_len);
	int (*fill_dst) (struct xfrm_dst * xdst, struct net_device * dev);
};

extern int xfrm_policy_register_afinfo(struct xfrm_policy_afinfo *afinfo);
extern int xfrm_policy_unregister_afinfo(struct xfrm_policy_afinfo *afinfo);
extern void km_policy_notify(struct xfrm_policy *xp, int dir,
			     struct km_event *c);
extern void km_state_notify(struct xfrm_state *x, struct km_event *c);

struct xfrm_tmpl;
extern int km_query(struct xfrm_state *x, struct xfrm_tmpl *t,
...
struct xfrm_state_afinfo {
	...
	int (*tmpl_sort) (struct xfrm_tmpl ** dst, struct xfrm_tmpl ** src,
			  int n);
	int (*state_sort) (struct xfrm_state ** dst, struct xfrm_state ** src,
			   int n);
...
extern int xfrm_register_type(const struct xfrm_type *type,
			      unsigned short family);
extern int xfrm_unregister_type(const struct xfrm_type *type,
				unsigned short family);
...
static inline struct xfrm_mode *xfrm_ip2inner_mode(struct xfrm_state *x,
						   int ipproto)
{
...
struct xfrm_mgr {
	...
	struct xfrm_policy *(*compile_policy) (struct sock * sk, int opt,
	...
	int (*report) (struct net * net, u8 proto, struct xfrm_selector * sel,
		       xfrm_address_t * addr);
	int (*migrate) (struct xfrm_selector * sel, u8 dir, u8 type,
...
static __inline__ __be16 xfrm_flowi_sport(struct flowi *fl)
{
	__be16 port;
	switch (fl->proto) {
	case IPPROTO_TCP:
...
static __inline__ __be16 xfrm_flowi_dport(struct flowi *fl)
{
	__be16 port;
	switch (fl->proto) {
	case IPPROTO_TCP:
 ...
 */
static inline int xfrm_sec_ctx_match(struct xfrm_sec_ctx *s1,
				     struct xfrm_sec_ctx *s2)
{
	return ((!s1 && !s2) ||
		(s1 && s2 &&
		 (s1->ctx_sid == s2->ctx_sid) &&
		 (s1->ctx_doi == s2->ctx_doi) && (s1->ctx_alg == s2->ctx_alg)));
}
#else
static inline int xfrm_sec_ctx_match(struct xfrm_sec_ctx *s1,
				     struct xfrm_sec_ctx *s2)
...
static inline struct sec_path *secpath_get(struct sec_path *sp)
{
	if (sp)
...
static inline void secpath_put(struct sec_path *sp)
{
...
static inline int xfrm_addr_any(xfrm_address_t * addr, unsigned short family)
{
	switch (family) {
	case AF_INET:
		return addr->a4 == 0;
...
{
	return (tmpl->saddr.a4 && tmpl->saddr.a4 != x->props.saddr.a4);
}
...
__xfrm6_state_addr_cmp(struct xfrm_tmpl *tmpl, struct xfrm_state *x)
{
	return (!ipv6_addr_any((struct in6_addr *)&tmpl->saddr) &&
		ipv6_addr_cmp((struct in6_addr *)&tmpl->saddr,
			      (struct in6_addr *)&x->props.saddr));
}

static inline int
xfrm_state_addr_cmp(struct xfrm_tmpl *tmpl, struct xfrm_state *x,
		    unsigned short family)
{
	switch (family) {
	case AF_INET:
...
static inline int xfrm_route_forward(struct sk_buff *skb, unsigned short family)
{
	...
	return !net->xfrm.policy_count[XFRM_POLICY_OUT] ||
	    (skb->dst->flags & DST_NOXFRM) || __xfrm_route_forward(skb, family);
}
			 ...
			 xfrm_address_t * daddr, xfrm_address_t * saddr)
{
	if (daddr->a4 == x->id.daddr.a4 &&
	    (saddr->a4 == x->props.saddr.a4 || !saddr->a4
	     || !x->props.saddr.a4))
		return 1;
	return 0;
}
...
{
	if (!ipv6_addr_cmp
	    ((struct in6_addr *)daddr, (struct in6_addr *)&x->id.daddr)
	    &&
	    (!ipv6_addr_cmp
	     ((struct in6_addr *)saddr, (struct in6_addr *)&x->props.saddr)
...
extern struct xfrm_state *xfrm_state_lookup_byaddr(struct net *net,
						   xfrm_address_t * daddr,
						   xfrm_address_t * saddr,
						   u8 proto,
						   unsigned short family);
...
static inline int xfrm_state_sort(struct xfrm_state **dst,
				  struct xfrm_state **src, int n,
				  unsigned short family)
{
	return -ENOSYS;
}
...
extern int xfrm_state_flush(struct net *net, u8 proto,
...
extern int xfrm_policy_walk(struct net *net, struct xfrm_policy_walk *walk,
			    int (*func) (struct xfrm_policy *, int, int,
					 void *), void *);
...
struct xfrm_policy *xfrm_policy_bysel_ctx(struct net *net, u8 type, int dir,
					  ...
					  int *err);
struct xfrm_policy *xfrm_policy_byid(struct net *net, u8, int dir, u32 id,
				     int delete, int *err);
...
extern int xfrm_sk_policy_insert(struct sock *sk, int dir,
				 struct xfrm_policy *pol);
...
extern int km_new_mapping(struct xfrm_state *x, xfrm_address_t * ipaddr,
			  __be16 sport);
extern void km_policy_expired(struct xfrm_policy *pol, int dir, int hard,
			      u32 pid);
extern int km_report(struct net *net, u8 proto, struct xfrm_selector *sel,
		     xfrm_address_t * addr);

extern void xfrm_input_init(void);
extern int xfrm_parse_spi(struct sk_buff *skb, u8 nexthdr, __be32 * spi,
			  __be32 * seq);
