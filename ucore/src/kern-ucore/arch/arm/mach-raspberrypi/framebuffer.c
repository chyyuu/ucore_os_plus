...
static void newline()
{
  ...
  if(consy<(max_y-1)) {
    ...
    memset((void *)(screenbase + (max_y-1)*rowbytes), 0, rowbytes);
  }
}
...
void fb_write(unsigned char ch) //char *text)
{
  ...
  if (ch == 13) {
    ...
    for(row=0; row<TELETEXT_H; row++) {
      unsigned int addr = (row+consy*TELETEXT_H)*pitch + consx*TELETEXT_W*2;
      volatile unsigned short int *ptr = (unsigned short int *)(screenbase+addr);
