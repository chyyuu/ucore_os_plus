...
static void
forkret(void) {//kprintf(" [forkrets pid=%d epc=%x] ", current->pid, current->tf->tf_epc);
/*    kprintf("{");
    int i;
...
int
copy_thread(uint32_t clone_flags,struct proc_struct *proc, uintptr_t esp, struct trapframe *tf) {
...
kernel_execve(const char *name, const char **argv,const char** kenvp) {
    ...
    asm volatile(
      ...
      "move %0, $v0;\n"
      : "=r"(ret)
      : "i"(/*T_SYSCALL+*/SYS_exec), "r"(name), "r"(argv), "r"(kenvp), "r"(argc) 
      : "a0", "a1", "a2", "a3", "v0"
    );
    return ret;
}
