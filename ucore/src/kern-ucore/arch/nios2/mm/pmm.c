...
page_init(void) {
    ...
    //init_memmap for sram
    {
        ...
        init_memmap(pa2page(begin_sram), (end_sram - begin_sram) / PGSIZE);
    }
    
    //init_memmap for sdram
    {
        uintptr_t begin_sdram = PADDR((uintptr_t)pages + sizeof(struct Page) * npage);
        uintptr_t end_sdram = SDRAM_END + 1;
        begin_sdram = ROUNDUP(begin_sdram, PGSIZE);
        end_sdram = ROUNDDOWN(end_sdram, PGSIZE);
        init_memmap(pa2page(begin_sdram), (end_sdram - begin_sdram) / PGSIZE);
    }
}
...
page_remove_pte(pde_t *pgdir, uintptr_t la, pte_t *ptep) {
	...
	if (*ptep & PTE_P) {// check if page directory is present
		...
		if (cnt == 0) free_pages(p, 1);// and free it when reach 0
