...
nios2_timer_init(void) {
	IOWR_ALTERA_AVALON_TIMER_CONTROL(na_timer, ALTERA_AVALON_TIMER_CONTROL_ITO_MSK
...
nios2_timer_usleep(int usec) {
    ...
    IOWR_ALTERA_AVALON_TIMER_CONTROL(na_timer1, ALTERA_AVALON_TIMER_CONTROL_START_MSK);
	while (1) {
		if (IORD_ALTERA_AVALON_TIMER_STATUS(na_timer1) & ALTERA_AVALON_TIMER_STATUS_TO_MSK) {
			break;
		}
	}
}
