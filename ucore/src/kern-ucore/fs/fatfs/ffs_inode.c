...
#if PRINTFSINFO
			FAT_PRINTF("[lookup_ffs_nolock] find defined node, path = %s, value = %d\n", path, hashValue);
#endif			
...
ffs_load_inode(struct ffs_fs *ffs, struct inode **node_store, TCHAR *path, struct ffs_inode* parent) {
    ...
    /* chrinstr */
		...
		if ((result = f_stat(actualPath, &fno)) != FR_OK) {
			FAT_PRINTF("[ffs_load_inode] get stat failed, f_stat %s %d\n", absPath, result);
			kfree(absPath);
			goto failed_cleanup_din;
			return ret;
		} else {
#if PRINTFSINFO
			FAT_PRINTF("[ffs_load_inode] get stat success, f_stat %s", absPath);
#endif
		}
		kfree(absPath);
