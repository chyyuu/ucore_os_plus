...
static void
mmc0_read_blks_nolock(uint32_t blkno, uint32_t nblks) {
    int ret;
    uint32_t sectno = blkno * MMC0_BLK_NSECT, nsecs = nblks * MMC0_BLK_NSECT;
    if ((ret = ide_read_secs(MMC0_DEV_NO, sectno, mmc0_buffer, nsecs)) != 0) {
        panic("mmc0: read blkno = %d (sectno = %d), nblks = %d (nsecs = %d): 0x%08x.\n",
                blkno, sectno, nblks, nsecs, ret);
    }
}

static void
mmc0_write_blks_nolock(uint32_t blkno, uint32_t nblks) {
    int ret;
    uint32_t sectno = blkno * MMC0_BLK_NSECT, nsecs = nblks * MMC0_BLK_NSECT;
    if ((ret = ide_write_secs(MMC0_DEV_NO, sectno, mmc0_buffer, nsecs)) != 0) {
        panic("mmc0: write blkno = %d (sectno = %d), nblks = %d (nsecs = %d): 0x%08x.\n",
                blkno, sectno, nblks, nsecs, ret);
    }
}

static int
mmc0_io(struct device *dev, struct iobuf *iob, bool write) {
    ...
    while (resid != 0) {
        ...
        if (write) {
            ...
            assert(copied != 0 && copied <= resid && copied % MMC0_BLKSIZE == 0);
...
static void
mmc0_device_init(struct device *dev) {
