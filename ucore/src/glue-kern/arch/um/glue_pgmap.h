...
static inline pte_perm_t ptep_get_perm(pte_t * ptep, pte_t perm)
{
	return (*ptep) & perm;
}

static inline void ptep_set_perm(pte_t * ptep, pte_t perm)
{
	*ptep |= perm;
}
...
static inline void ptep_unset_perm(pte_t * ptep, pte_t perm)
{
	*ptep &= (~perm);
}
